...
public class Ssdeep {
    ...
    private long roll_hash(int c) {
        ...
        // in theory should have no effect. This AND has been removed for
        // performance
    ...
    private void ss_engine(Ss_context ctx, byte[] buffer, int buffer_size) {
        ...
        for (i = 0; i < buffer_size; i++) {
            ...
            // When the rolling hash hits the
            // reset value then we emit the normal hash as an element of the
            // signature and reset both hashes
            if (buffer[i] >= 0) {
            ...
            if (ctx.h % ctx.block_size == (ctx.block_size - 1)) {
                ...
                // on all characters in the
                // piece of the message between the last reset point and this
                // one
                ctx.p[ctx.j] = b64[(int) (ctx.h2 % 64)];
                if (ctx.j < SPAMSUM_LENGTH - 1) {
                    // we can have a problem with the tail overflowing. The
                    // easiest way to cope with this is to only reset the
                    // second hash if we have room for more characters in our
                    // signature. This has the effect of
                    // combining the last few pieces of the message into a
                    // single piece
                    ctx.h2 = HASH_INIT;
                    (ctx.j)++;
                }
            ...
            // block_size*2. By producing dual signatures in
            // this way the effect of small changes in the message size near a
            // block size boundary is greatly reduced.
    ...
    private int has_common_substring(byte[] s1, byte[] s2) {
        ...
        // compare it to all of the rolling hashes
        // for the first string. If one matches then we have a
        // candidate substring match. We then confirm that match with a direct
        // string comparison
     ...
     * @param idx2
     * @param rollingWindow
     * @return -1 : not equals 0 : equals 1 : array1 is not long enough 2 :
     * array2 is not long enough
     */
     ...
     * sequences contain very little information so they tend to just bias the
     * result unfairly
     *
    ...
    private long score_strings(byte[] s1, byte[] s2, long block_size) {
        ...
        // absolute quantity. It also copes with the variability of the string
        // lengths
     ...
     * Compare
     * <p>
     * given two spamsum signature return a value indicating the degree to which
     * they match.
     *
    ...
    public int Compare(String signa1, String signa2) {
        return Compare(new SpamSumSignature(signa1), new SpamSumSignature(signa2));
    }
